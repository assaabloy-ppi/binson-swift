...
class WhiteBoxTests: XCTestCase {
    ...
    func testNumbers() {
        let p_127: Data = packNumber(127)
        let p_128: Data = packNumber(128)
        let p_big: Data = packNumber(32767)

        let n_128: Data = packNumber(-128)
        let n_129: Data = packNumber(-129)
        let n_sma: Data = packNumber(-32768)
    ...
    func testBackForthNumbers() {
        for i in [posx10, negx10, posx11, negx11, posx12, negx12, posx13, negx13] {
            let v_low = packNumber(i.low)
            let v_high = packNumber(i.high)
